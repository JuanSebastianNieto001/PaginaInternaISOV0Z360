import type { Metadata } from "next";
import { Suspense } from "react";

import { ActivityTimeline } from "@/components/documents/activity-timeline";
import { ActivityFilters } from "@/components/activity/activity-filters";
import { PageHeader } from "@/components/ui/page-header";
import { Pagination } from "@/components/ui/pagination";
import { ErrorState } from "@/components/ui/states";
import { can } from "@/lib/auth/permissions";
import { requireUser } from "@/lib/auth/session";
import { PERMISSIONS } from "@/lib/constants/permissions";
import { listAuditLogs } from "@/lib/services/audit.service";
import { createClient } from "@/lib/supabase/server";
import { asDate, asInt, first, type SearchParams } from "@/lib/utils/url";

export const metadata: Metadata = { title: "Actividad" };

export default async function ActivityPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  const [user, params] = await Promise.all([requireUser(), searchParams]);
  const supabase = await createClient();
  const canReadAll = can(user, PERMISSIONS.AUDIT_READ);

  const action = first(params.action);
  const query = {
    action: action && /^[a-z_]+\.[a-z_]+$/.test(action) ? action : undefined,
    dateFrom: asDate(params.from),
    dateTo: asDate(params.to),
    page: asInt(params.page, 1, 1, 100_000),
    pageSize: 25,
  };

  let result;
  try {
    result = await listAuditLogs(supabase, query);
  } catch {
    return <ErrorState />;
  }

  return (
    <>
      <PageHeader
        title="Actividad"
        description={canReadAll ? "Registro cronológico de todas las acciones del sistema." : "Tu actividad y los cambios realizados sobre la documentación."}
      />
      <div className="space-y-4">
        <Suspense>
          <ActivityFilters action={query.action} dateFrom={query.dateFrom} dateTo={query.dateTo} />
        </Suspense>
        <ActivityTimeline logs={result.items} emptyTitle="Sin actividad" emptyDescription="No hay registros que coincidan con los criterios." />
        <Suspense>
          <Pagination page={result.page} totalPages={result.totalPages} total={result.total} pageSize={result.pageSize} itemLabel="registros" />
        </Suspense>
      </div>
    </>
  );
}
